...
class SeqDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        random_pos_item = target_pos[-1]
        random_neg_item = target_neg[-1]
        # random_pos_item = target_pos[np.random.randint(len(input_ids))]
        # random_neg_item = target_neg[np.random.randint(len(input_ids))]
        ...
        if self.test_neg_items is not None:
            ...
            cur_tensors = (
                ...
                torch.tensor(random_pos_item, dtype=torch.long),
                torch.tensor(random_neg_item, dtype=torch.long)
            )
