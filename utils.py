...
def check_path(path):
    ...
    if not os.path.exists(f'{path}/ckpt'):
        os.makedirs('{}/ckpt'.format(path))
        print(f'{path}/ckpt created')
...
def log_normal_mixture(z, m, v, mask=None):
    ...
    # 当 mask 非 0 即 1 时，给不存在的标签赋值一个非常小的概率，
...
def multi_interest_cl(user_interests_a, user_interests_b, temperature=0.1):
    '''
    :param x: bs * n_interest * embed_size
    :param y: bs * n_interest * embed_size
    :return:
    '''
    batch_size, n_interest, embed_size = user_interests_a.shape
    user_interests_a = F.normalize(user_interests_a, p=2, dim=-1).reshape(-1, embed_size)
    user_interests_b = F.normalize(user_interests_b, p=2, dim=-1).reshape(-1, embed_size)
    sim_matrix = user_interests_a.matmul(user_interests_b.transpose(0, 1)) / temperature
    clloss = F.cross_entropy(sim_matrix, torch.arange(sim_matrix.shape[0]).cuda()) + F.cross_entropy(
        sim_matrix.transpose(0, 1), torch.arange(sim_matrix.shape[0]).cuda())
    return clloss
